...
brach_data<-read.csv("data/raw/brachP_test_data.csv")
aorta_data<-read.csv("data/raw/aortaP_test_data.csv")

###make an unmodified dataset to get column order correct below###
merged<-merge(brach_data,aorta_data, by=c('X')) %>% arrange(X) %>% rename(subject="X")
...
export_spline_wide_data<-export_spline %>% mutate(record=paste0(dataset,"_t_",as.character(time)))%>%
  dcast(subject ~record, value.var="spline")
...
write.csv(export_spline_final,"Data/final/test_models_spline.csv",row.names=F)
